                        ...
                        <tbody style="vertical-align: middle">
                            @if ($pkpmk->isNotEmpty())
                                @foreach ($pkpmk as $data => $item)
                                    <tr wire:key='{{ sha1($item->id_pkpmk) }}'>
                                        ...
                                        <td>{{ $item->no_pkpmk ?? '-' }}</td>
                                        <td>{{ $item->tgl_pkpmk?->translatedFormat('d M Y, H:i') ?? '-' }}</td>
                                        <td>
                                            {{-- Status --}}
                                        ...
                                        <td>
                                            <a href="{{ route('pkpmk.show', encrypt($item->id_pkpmk)) }}"
...
@section('script')
    ...
    <script>
        ...
        $('#spk').on("change", function() {
            let id = $(this).val();
            window.location.href = '/pkpmk/add/' + id;
        })
    </script>
