...
for (let i = 1; i < 200; i++) {
    ...
    const kes_taksasi_persen2 = document.getElementById(
    ...
    if (pasarSF) {
        pasarSF.addEventListener("keyup", function () {
            setNilaiSM(i);
        });
    }

    if (kendaSF) {
        kendaSF.addEventListener("keyup", function () {
            setHargaDiterima(i);
        });
    }

    if (kes_taksasi_persen) {
        kes_taksasi_persen.addEventListener("keyup", function () {
            NilaiPasarAgunan(i);
        });
    }

    if (kes_taksasi_persen2) {
        kes_taksasi_persen2.addEventListener("keyup", function () {
            NilaiPasarAgunan(i);
        });
    }
...
function AutoLuas(i) {
    ...
    if (luas_tanah) {
        luas_tanah.addEventListener("keyup", function () {
            [dl_1, dl_2, dl_3].forEach((el, idx) => {
                if (el) {
                    el.value = luas_tanah.value;
                    el.classList.remove("is-invalid");
                    el.classList.add("is-valid");

                    // hitung ulang total tanah untuk index terkait
                    const inpRp = document.getElementById(
                        `data_${idx + 1}_${i}`,
                    );
                    const inpTotal = document.getElementById(
                        `data_total_${idx + 1}_${i}`,
                    );
                    if (inpRp && inpTotal) {
                        const harga =
                            toNumber(inpRp.dataset.rawValue || inpRp.value) ||
                            0;
                        const luas = parseFloat(el.value) || 0;
                        inpTotal.value = setFormatRupiah(harga * luas);
                    }
                }
            });
        });
    ...
    if (luas_bangunan) {
        luas_bangunan.addEventListener("keyup", function () {
            [bg_1, bg_2, bg_3].forEach((el, idx) => {
                if (el) {
                    el.value = luas_bangunan.value;
                    el.classList.remove("is-invalid");
                    el.classList.add("is-valid");

                    // hitung ulang total bangunan untuk index terkait
                    const inpRp = document.getElementById(
                        `bangunan_${idx + 1}_${i}`,
                    );
                    const inpTotal = document.getElementById(
                        `bangunan_total_${idx + 1}_${i}`,
                    );
                    if (inpRp && inpTotal) {
                        const harga =
                            toNumber(inpRp.dataset.rawValue || inpRp.value) ||
...
function NilaiPasarAgunan(i) {
    ...
    if (inpPasar && inpLuas) {
        ...
        const totalScore = document.getElementById(`total_score_tanah_${i}`);
        ...
        if (totalScore) {
            ...
            if (kategori_jaminan.value == "Ruko & Rukan") {
                // rukan
                const totalScoreR = document.getElementById(`total_score_${i}`);
            ...
            // for kesimpulan
            const kes_pasar = document.getElementById(`kes_nilai_pasar_${i}`);
            const kes_taksasi_persen = document.getElementById(
                `kes_nilai_taksasi_persen_${i}`,
            );
            ...
            let total_kes_taksasi =
                total_kes_pasar * (parseFloat(kes_taksasi_persen.value) / 100);
    ...
    if (pasarBangunan) {
        ...
        const harga =
            toNumber(pasarBangunan.dataset.rawValue || pasarBangunan.value) ||
            0;
        ...
        if (scoreBangunan && kategori_jaminan.value == "Tanah & Bangunan") {
            ...
            const pasar_tanah = document.getElementById(`kes_nilai_pasar_${i}`);
