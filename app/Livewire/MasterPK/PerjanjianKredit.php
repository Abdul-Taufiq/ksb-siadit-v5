...
class PerjanjianKredit extends Component
{
    use WithPagination, WithoutUrlPagination, WithFileUploads;
    // For filter
    public $sortBy = 'created_at', $sortDir = 'desc', $search = '', $perPage = 10;
    public $kc = false, $id_cabang, $id_cab_area, $tgl_awal,  $tgl_akhir, $id_area_1, $id_area_2, $id_area_3;
    // for modal
    public $modal_title, $spk = [], $status, $catatan, $keterangan_kaops = 0, $id;
    // load services
    protected PkServices $pk_services;
    public function boot(PkServices $pk_services)
    {
    ...
    public function ShowModal($metode, $id)
    {
        ...
        if ($metode == 'Add') {
            ...
            $pkpmk = PkPmk::find(base64_decode($id));
    ...
    public function SendKaops($status, $id)
    {
        ...
        // USER PIMPINAN CABANG
        if ($user->jabatan == 'Pimpinan Cabang') {
            // update kredit
            $kredit = Kredit::find($pkpmk->id_kredit);
            $kredit->update([
                'status_pincab' => 'Approve',
                'status_kredit' => 'Pimpinan Cabang Approve',
            ]);

            // Log Aktivitas
            LogActivity::AddLog("(*) S.O.S Disetujui | SPK: {$pkpmk->kredit->no_spk} | NIK: {$pkpmk->debitur->nik} | Nama: {$pkpmk->debitur->nama_debitur}");
    ...
    public function render()
    {
        ...
        $pkpmk = $this->pk_services->index($this->all());
        // untuk mengecualikan error
        /** @disregard P1013 Undefined method */
        return view('livewire.master-p-k.perjanjian-kredit', compact('pkpmk'))
            ->extends('livewire.komponen.layouts.app', ['title' => 'Data Perjanjian Kredit'])
