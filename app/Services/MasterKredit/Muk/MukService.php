...
class MukService
{
    public function index($data)
    {
        ...
        $query = Muk::with('kredit')
            // ->when($data['id_cabang'] != 99, function ($q) use ($data) {
            //     $q->where('tb_muk.id_cabang', $data['id_cabang']);
            // })
            ...
            ->when($data['tgl_awal'] && $data['tgl_akhir'], function ($q) use ($data) {
                ...
                $q->whereBetween('tgl_muk', [$awal, $akhir]);
            });
        ...
        if ($id_cabang == 20) {
            if (!empty($data['id_cabang'])) {
                if ($data['id_cabang'] == 'AREA 1' || $data['id_cabang'] == 'AREA 2' || $data['id_cabang'] == 'AREA 3') {
                    $query->whereIn('tb_muk.id_cabang', $data['id_cab_area']);
                } else {
                    $query->where('tb_muk.id_cabang', $data['id_cabang']);
                }
            } else {
                $query->whereIn('tb_muk.id_cabang', $data['id_cab_area']);
            }
            ...
            switch ($data['id_cabang']) {
                case 'AREA 1':
                    # code...
                    $query->whereIn('tb_muk.id_cabang', $data['id_area_1']);
                    break;
                case 'AREA 2':
                    # code...
                    $query->whereIn('tb_muk.id_cabang', $data['id_area_2']);
                    break;
                case 'AREA 3':
                    # code...
                    $query->whereIn('tb_muk.id_cabang', $data['id_area_3']);
                    break;
            }
        } else {
        ...
        if ($data['sortBy'] === 'no_spk') {
            $query->join('tb_kredit', 'tb_muk.id_kredit', '=', 'tb_kredit.id_kredit')
    ...
    public function storePartSatu($data)
    {
        ...
        if ($kredit->kategori_spk == 'Restruck') {
            ...
            if ($ambil->isEmpty()) {
                $urut = "001";
                $nomer = $urut . '/KSB.' . $cabang . '/MUK-KRD/NR/' . $bulanRom . '/' . $thn;
            } else {
                foreach ($ambil as $item) {
                    $cekTahun = substr($item->no_muk, -4, 4);
                    if ($cekTahun != $thn) {
    ...
    public function storePartDua($data)
    {
        // Data Management
        $manag = MukData::create([
    ...
    public function storeTanah($data)
    {
        $id_kredit = base64_decode($data['id_kredit']);
        $id_muk = base64_decode($data['id_muk']);
        ...
        for ($i = 1; $i < 201; $i++) {
            if (!empty($data['nama_deb_' . $i])) {
                ...
                if (Auth::user()->sub_jabatan == 'Staf Analis Area') {
                    ...
                    if (isset($data['bangunan_luas_1_' . $i]) && $data['bangunan_luas_1_' . $i] !== '') {
    ...
    // store sc kendaraan
    public function storeKenda($data)
    {
        $id_kredit = base64_decode($data['id_kredit']);
        $id_muk = base64_decode($data['id_muk']);
        $nama_user = Auth::user()->nama;
    ...
    // store Deposito & tabungan
    public function storeDepo($data)
    {
        $id_kredit = base64_decode($data['id_kredit']);
        $id_muk = base64_decode($data['id_muk']);
        $nama_user = Auth::user()->nama;
