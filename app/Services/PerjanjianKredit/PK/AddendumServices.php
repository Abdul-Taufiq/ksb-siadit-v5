...
class AddendumServices
{
    public function index($data)
    {
        ...
        $query = PkPmkAddendum::with(['kredit', 'debitur'])
            // ->when($data['id_cabang'] != 99, function ($q) use ($data) {
            //     $q->where('tb_pkpmk_addendum.id_cabang', $data['id_cabang']);
            // })
            ...
            ->when($data['tgl_awal'] && $data['tgl_akhir'], function ($q) use ($data) {
                ...
                $q->whereBetween('tgl_addendum', [$awal, $akhir]);
            });
        ...
        if ($id_cabang == 20) {
            if (!empty($data['id_cabang'])) {
                if ($data['id_cabang'] == 'AREA 1' || $data['id_cabang'] == 'AREA 2' || $data['id_cabang'] == 'AREA 3') {
                    $query->whereIn('tb_pkpmk_addendum.id_cabang', $data['id_cab_area']);
                } else {
                    $query->where('tb_pkpmk_addendum.id_cabang', $data['id_cabang']);
                }
            } else {
                $query->whereIn('tb_pkpmk_addendum.id_cabang', $data['id_cab_area']);
            }
            ...
            switch ($data['id_cabang']) {
                case 'AREA 1':
                    # code...
                    $query->whereIn('tb_pkpmk_addendum.id_cabang', $data['id_area_1']);
                    break;
                case 'AREA 2':
                    # code...
                    $query->whereIn('tb_pkpmk_addendum.id_cabang', $data['id_area_2']);
                    break;
                case 'AREA 3':
                    # code...
                    $query->whereIn('tb_pkpmk_addendum.id_cabang', $data['id_area_3']);
                    break;
            }
        } else {
        ...
        if ($data['sortBy'] === 'no_spk') {
            $query->join('tb_kredit', 'tb_pkpmk_addendum.id_kredit', '=', 'tb_kredit.id_kredit')
    ...
    public function AddendumStore($data)
    {
        ...
        if (!empty($data['id_addendum'])) {
            $pkpmk = PkPmkAddendum::find(base64_decode($data['id_addendum']));
            $pkpmk->update([
                'id_cabang' => Auth::user()->id_cabang,
                ...
                'no_akta_notaris' => $data['nomor_akta_notaris'],
        ...
        for ($i = 1; $i <= 50; $i++) {
            ...
            if (!empty($data['id_tanah_' . $i])) {
                ...
                $jaminan_tanah->kategori_pengikatan = $data['kategori_pengikatan_' . $i];

                if ($data['kategori_pengikatan_' . $i] == 'Sudah ada SKMHT') {
                    $jaminan_tanah->jns_perikatan = 'SKMHT';
                } else {
                    $jaminan_tanah->jns_perikatan = $data['jns_perikatan_' . $i];
                }

                $jaminan_tanah->no_akta_perikatan = $data['no_akta_perikatan_' . $i];
                $jaminan_tanah->tgl_akta_perikatan = $data['tgl_akta_perikatan_' . $i];
            ...
            if (!empty($data['id_kenda_' . $i])) {
                ...
                $jaminan_kenda->kategori_pengikatan = $data['kategori_pengikatan_fidusia_' . $i] ?? null;
                $jaminan_kenda->no_akta_perikatan = $data['no_akta_perikatan_fidusia_' . $i] ?? null;
                $jaminan_kenda->save();
            }
        }
    ...
    // generate no SPPK
    public function genetareSppk($id)
    {
        ...
        if ($pkpmk->no_sppk === null) {
            ...
            if ($kredit->kategori_spk == 'Restruck') {
                ...
                $now = Carbon::now();
                $thn = $now->year;
                $ambil = PkPmkAddendum::where('no_sppk', 'LIKE', "%/KSB." . $cabang . "/SPPK/R/%")
                    ->orderBy('updated_at', 'desc')
                    ->take(1)
                    ->get();
                if ($ambil->isEmpty()) {
                    $urut = "0001";
                    $nomer = $urut . '/KSB.' . $cabang . '/SPPK/R/' . $bulanRom . '/' . $thn;
                } else {
            ...
            // NON-RESTRUCK
                ...
                $now = Carbon::now();
                $thn = $now->year;
                $ambil = PkPmkAddendum::where('no_sppk', 'LIKE', "%/KSB." . $cabang . "/SPPK/NR/%")
                    ->orderBy('updated_at', 'desc')
                    ->take(1)
                    ->get();
                if ($ambil->isEmpty()) {
                    $urut = "0001";
                    $nomer = $urut . '/KSB.' . $cabang . '/SPPK/NR/' . $bulanRom . '/' . $thn;
                } else {
                    foreach ($ambil as $item) {
                        $cekTahun = substr($item->no_sppk, -4, 4);
                        if ($cekTahun != $thn) {
    ...
    public function genetarePk($id)
    {
        ...
        if ($pkpmk->no_addendum == '') {
            ...
            if ($kredit->kategori_spk == 'Restruck') {
                ...
                if ($ambil->isEmpty()) {
                    $urut = "0001";
                    $nomer = $urut . '/KSB.KRD-' . $cabang . '/R/' . $bulanRom . '/' . $thn;
                } else {
                    foreach ($ambil as $item) {
                        $cekTahun = substr($item->no_addendum, -4, 4);
                        if ($cekTahun != $thn) {
                            $urut = "0001";
                            $nomer = $urut . '/KSB.KRD-' . $cabang . '/R/' . $bulanRom . '/' . $thn;
                        } else {
                            $urut = substr($item->no_addendum, 0, 4);
                            $urut = (int)$urut + 1;
                ...
                if ($ambil->isEmpty()) {
                    $urut = "0001";
                    $nomer = $urut . '/KSB.KRD-' . $cabang . '/NR/' . $bulanRom . '/' . $thn;
                } else {
                    foreach ($ambil as $item) {
                        $cekTahun = substr($item->no_addendum, -4, 4);
                        if ($cekTahun != $thn) {
            ...
            $pkpmk->no_addendum = $nomer;
            $pkpmk->tgl_addendum = now();
            $pkpmk->tgl_awal = now();
    ...
    public function genetareTpbj($id, $type)
    {
        ...
        $jam_kenda = JamKenda::where('id_kredit', $kredit->id_kredit)->get();
        $jam_tanah = JamTanah::where('id_kredit', $ids)->get();
